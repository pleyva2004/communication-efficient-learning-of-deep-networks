...
> **Remark 3.1 (the heuristic $\Delta$ is a theorem — with a corrected prefactor).** The two-step Taylor sketch above is *heuristic*: it freezes the local learning rate at $\eta$ and stops at $\tau=2$. An effective-ODE / averaging-theory argument makes it rigorous. Idealize client $k$'s $u_k$ local full-batch steps as the gradient flow $\Phi_k^T$ of $F_k$ run for physical time $T=\eta u_k$; then the FedAvg server map $\mathcal S^T=\sum_k\beta_k\Phi_k^T$ drifts from the centralized flow $\Phi_f^T$ by exactly $\|\mathcal S^T(w)-\Phi_f^T(w)\|=\tfrac{T^2}{2}\|\mathrm{Cov}_k(H_k,g_k)\|+O(T^3)$, with leading direction $\mathrm{Cov}_k(H_k,g_k)/\|\cdot\|$. This recovers (3.5)'s drift object $\mathrm{Cov}_k(H_k,g_k)$ but **corrects its prefactor**: the right magnitude is $T^2/2=(\eta u_k)^2/2$, not the frozen $\eta^2$, so relative to the raw $\eta^2$ heuristic the drift grows by a factor $u_k^2/2$ (e.g. $\approx 12.5\times$ at $E{=}5$). At the $E{=}2,B{=}\infty$ ($T{=}2\eta$) calibration point the flow gives $2\eta^2$ — twice (3.5)'s $\eta^2$ — the factor $2$ being exactly the explicit-Euler discretization error of the two-step sketch. The single invariant is $T=\eta E n_k/B$. See [`05-improvements.tex` §T.3](05-improvements.tex) and the full proof in [`proofs/effective-ode-averaging-bound.tex`](proofs/effective-ode-averaging-bound.tex) (numerically confirmed: drift $\propto T^{2.06}$, direction cosine $0.9999$).
